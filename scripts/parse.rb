...
def estimate(es,total)
  ...
  %w(w1 w2 ).each do |i|
   begin
    d = es[" result_detail_#{i}_redis"].to_i
...
def change_value(arr)
  arr.each do |h|
    %w(w1 w2).each do |i|
      ...
      if h['destinationName'] == " result_detail_#{i}"
        h["QueueSize"] = $r.llen("SCT_CRAWL_zht_detail_#{i}_html") 
      end
